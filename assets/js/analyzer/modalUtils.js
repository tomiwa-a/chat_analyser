...
export function getFilteredMessages(
  ...
  if (!allParticipants) {
    ...
    if (selectedParticipants.length > 0) {
      filtered = filtered.filter((m) =>
        selectedParticipants.includes(m.author)
      );
    }
  }
...
export function setupAllParticipantsToggle(
  allCheckboxId,
  ...
  newAllCheckbox.addEventListener("change", () => {
    ...
    individualCheckboxes.forEach((cb) => {
      cb.disabled = newAllCheckbox.checked;
  ...
  // Listen to individual checkboxes to uncheck "All" if any individual is selected
  const individualCheckboxes = document.querySelectorAll(
    `.${individualCheckboxClass}`
  );
  individualCheckboxes.forEach((checkbox) => {
    checkbox.addEventListener("change", () => {
      const anyChecked = document.querySelector(
        `.${individualCheckboxClass}:checked`
      );
      if (anyChecked) {
        newAllCheckbox.checked = false;
      } else {
        // If no individual is checked, check "All" again
